...
class CLSWP_FieldType
{
    ...
    public function __construct(string $lbl, string $type, string $defaultValue, array $opts = [])
    {
        $this->label = $lbl;
    ...
    public function render()
    {
        ...
        switch ($this->inType) {
            case 'text': ?>
                <input style="width: 330px;" type='text' class="cl-fld-text" id='<?php echo esc_attr($this->fldId) ?>' name="<?php echo esc_attr($this->fldId) ?>" value="<?php echo esc_attr((get_option($this->fldId, $this->defaultValue))) ?>" />
            <?php
                break;
            case 'number': ?>
